  ...
  renderTimeline() {
    ...
    var filtered = companies.filter(function(c) {
      ...
      if (self.timelineFilters.go) {
        var go = self.goStatus[c] || "go";
        if (go !== self.timelineFilters.go) return false;
      }
      return true;
    });

    // Determine milestone reached per company
    function getMilestoneInfo(company) {
      var items = self.interactions.filter(function(i) { return i.company === company; });
      if (!items.length) return { index: -1, key: "none", pct: 0, interactions: [] };
      ...
      // 1st Contact
      if (items.length >= 1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "1st"; }));
      }
      // Meeting
      if (items.some(function(i) { return (i.type || "").toLowerCase() === "meeting" || allText.indexOf("meeting") !== -1; })) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "meeting"; }));
      }
      // NDA
      if (allText.indexOf("nda") !== -1 || allText.indexOf("non-disclosure") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "nda"; }));
      }
      // EOI
      if (allText.indexOf("eoi") !== -1 || allText.indexOf("expression of interest") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "eoi"; }));
      }
      // Technical Discussion
      if (allText.indexOf("technical") !== -1 || allText.indexOf("tech discussion") !== -1 || allText.indexOf("technical review") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "tech"; }));
      }
      // Evaluation Started
      if (allText.indexOf("eval") !== -1 || allText.indexOf("review started") !== -1 || allText.indexOf("evaluation started") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "eval_started"; }));
      }
      // Evaluation One
      if (allText.indexOf("eval one") !== -1 || allText.indexOf("eval 1") !== -1 || allText.indexOf("first evaluation") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "eval_one"; }));
      }
      // Evaluation Two
      if (allText.indexOf("eval two") !== -1 || allText.indexOf("eval 2") !== -1 || allText.indexOf("second evaluation") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "eval_two"; }));
      }
      // Shortlisted
      if (allText.indexOf("shortlist") !== -1 || allText.indexOf("short list") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "shortlisted"; }));
      }
      // Selected
      if (allText.indexOf("selected") !== -1 || allText.indexOf("awarded") !== -1 || allText.indexOf("contract") !== -1) {
        mi = Math.max(mi, MILESTONES.findIndex(function(m) { return m.key === "selected"; }));
      }

      var pct = mi >= 0 ? Math.round(((mi + 1) / MILESTONES.length) * 100) : 0;
      return { index: mi, key: mi >= 0 ? MILESTONES[mi].key : "none", pct: pct, interactions: sorted };
    }

    // Compute and sort (longer bar = top)
    var rows = filtered.map(function(c) {
      return { company: c, bus: busMap[c], info: getMilestoneInfo(c) };
    });
    rows.sort(function(a, b) { return b.info.pct - a.info.pct || a.company.localeCompare(b.company); });

    // Render milestone header
    var headerEl = document.getElementById("tlMilestoneHeader");
    if (headerEl) {
      var mhtml = '<div class="tl-prog-row tl-prog-header">';
      mhtml += '<div class="tl-prog-name"></div>';
      mhtml += '<div class="tl-prog-bar-area">';
      mhtml += '<div class="tl-prog-track tl-prog-track-header">';
      MILESTONES.forEach(function(m, i) {
        var left = Math.round(((i + 0.5) / MILESTONES.length) * 100);
        var color = MILESTONE_COLORS[m.key] || "#888";
        mhtml += '<span class="tl-ms-label" style="left:' + left + '%;color:' + color + '" data-ms-index="' + i + '">' + self.escapeHtml(m.label) + '</span>';
        mhtml += '<span class="tl-ms-dot-marker" style="left:' + left + '%;background:' + color + '"></span>';
      });
      mhtml += '</div><div class="tl-prog-pct"></div></div></div>';
      headerEl.innerHTML = mhtml;

      // Bind double-click to edit milestone label
      headerEl.querySelectorAll(".tl-ms-label").forEach(function(label) {
        label.addEventListener("dblclick", function(e) {
          e.stopPropagation();
          var idx = parseInt(label.dataset.msIndex);
          var current = MILESTONES[idx].label;
          var input = document.createElement("input");
          input.type = "text";
          input.value = current;
          input.className = "tl-ms-input";
          label.replaceWith(input);
          input.focus();
          input.select();
          var finish = function() {
            var val = input.value.trim() || current;
            self.milestoneLabels[idx].label = val;
            self.saveMilestoneLabels();
            self.renderTimeline();
          };
    ...
    rows.forEach(function(row) {
      ...
      var bus = row.bus;
      var info = row.info;
      var color = MILESTONE_COLORS[info.key] || MILESTONE_COLORS["none"];
      var goClass = self.goStatus[c] === "nogo" ? " tl-prog-nogo" : "";
      var latest = info.interactions.length ? info.interactions[info.interactions.length - 1] : null;
      var latestDate = latest ? latest.date : "—";
      var latestType = latest ? (latest.type || "") : "";

      var tipLines = [];
      tipLines.push("Stage: " + (info.index >= 0 ? MILESTONES[info.index].label : "No activity"));
      tipLines.push("Last: " + latestDate + (latestType ? " (" + latestType + ")" : ""));
      tipLines.push("Interactions: " + info.interactions.length);
      var tooltip = tipLines.join("\n");

      html += '<div class="tl-prog-row' + goClass + '" data-company="' + self.escapeHtml(c) + '">';
      html += '<div class="tl-prog-name"><span class="tl-prog-company">' + self.escapeHtml(c) + '</span><span class="tl-prog-country">' + self.escapeHtml(bus ? bus.country || "" : "") + '</span></div>';
      html += '<div class="tl-prog-bar-area">';
      html += '<div class="tl-prog-track">';
      html += '<div class="tl-prog-fill" data-ms-fill="' + c + '" style="width:' + info.pct + '%;background:' + color + '"></div>';

      MILESTONES.forEach(function(m, i) {
        var left = Math.round(((i + 0.5) / MILESTONES.length) * 100);
        var reached = i <= info.index;
        var dotColor = MILESTONE_COLORS[m.key] || "#888";
        html += '<span class="tl-ms-dot' + (reached ? " tl-ms-reached" : "") + '" data-ms-dot="' + c + '" data-ms-i="' + i + '" style="left:' + left + '%;' + (reached ? "background:" + dotColor + ";border-color:" + dotColor : "") + '"></span>';
      });

      // Slider input overlaid on the track
      html += '<input type="range" class="tl-prog-slider" min="0" max="' + (MILESTONES.length - 1) + '" value="' + Math.max(0, info.index) + '" data-company="' + self.escapeHtml(c) + '">';

      html += '</div>';
      html += '<div class="tl-prog-pct" data-ms-pct="' + c + '">' + (info.pct > 0 ? info.pct + "%" : "—") + '</div>';
      html += '</div></div>';
    });
    ...
    // Bind sliders
    el.querySelectorAll(".tl-prog-slider").forEach(function(slider) {
      slider.addEventListener("input", function() {
        var company = slider.dataset.company;
        var val = parseInt(slider.value);
        var msCount = MILESTONES.length;
        var pct = val >= 0 ? Math.round(((val + 1) / msCount) * 100) : 0;

        // Update fill bar
        var fill = el.querySelector('[data-ms-fill="' + company + '"]');
        if (fill) {
          var msKey = val >= 0 ? MILESTONES[val].key : "none";
          var fillColor = MILESTONE_COLORS[msKey] || MILESTONE_COLORS["none"];
          fill.style.width = pct + "%";
          fill.style.background = fillColor;
        }

        // Update dots
        el.querySelectorAll('[data-ms-dot="' + company + '"]').forEach(function(dot) {
          var dotI = parseInt(dot.dataset.msI);
          var reached = dotI <= val;
          var dotColor = MILESTONE_COLORS[MILESTONES[dotI].key] || "#888";
          if (reached) {
            dot.classList.add("tl-ms-reached");
            dot.style.background = dotColor;
            dot.style.borderColor = dotColor;
          } else {
            dot.classList.remove("tl-ms-reached");
        ...
        // Update percentage
        var pctEl = el.querySelector('[data-ms-pct="' + company + '"]');
        if (pctEl) pctEl.textContent = val >= 0 ? pct + "%" : "—";

        // Save override
        self.setCompanyStage(company, val >= 0 ? MILESTONES[val].key : "");
      });
    });
  },
