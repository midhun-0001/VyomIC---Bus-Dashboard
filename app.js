...
const APP = {
  ...
  stageLabels: { "new": "New (No Activity)", "1st": "1st Response", "2nd": "2nd Response", "3rd": "3rd+ Response", "nda": "NDA", "meeting": "Meeting", "eoi": "EOI" },
  goStatus: {}, // { [company]: "go" | "nogo" }
  timelineMode: "monthly", // "monthly" | "weekly"
  timelineFilters: { country: "", stage: "", go: "" },
  ...
  getCompanyStage(company) {
    ...
    var colors = { "new": "var(--color-text-muted)", "1st": "#74b9ff", "2nd": "#a29bfe", "3rd": "#fd79a8", "nda": "var(--color-secondary)", "meeting": "#00b894", "eoi": "#fdcb6e" };
  ...
  renderAnalysis() {
    var companies = [...new Set(this.buses.map(b => b.company))].sort();
    var busMap = {};
    this.buses.forEach(b => { if (!busMap[b.company]) busMap[b.company] = b; });
  ...
  renderTimeline() {
    ...
    var stageSelect = document.getElementById("tlFilterStage");
    ...
    if (stageSelect && stageSelect.options.length <= 1) {
      Object.keys(this.stageLabels).forEach(function(k) {
        var o = document.createElement("option"); o.value = k; o.textContent = APP.stageLabels[k]; stageSelect.appendChild(o);
      });
    }
    ...
    var filtered = companies.filter(function(c) {
      ...
      if (self.timelineFilters.stage) {
        var stage = self.getCompanyStage(c);
        if (stage.key !== self.timelineFilters.stage) return false;
      }
      return true;
    });

    // Determine time range: last 12 months
    var now = new Date();
    var periods = [];
    if (this.timelineMode === "monthly") {
      for (var m = 11; m >= 0; m--) {
        var d = new Date(now.getFullYear(), now.getMonth() - m, 1);
        periods.push({ key: d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0"), label: d.toLocaleString("en", { month: "short" }).substring(0, 3), year: d.getFullYear(), month: d.getMonth(), start: d, end: new Date(d.getFullYear(), d.getMonth() + 1, 0, 23, 59, 59) });
      }
    } else {
      // Weekly: last 26 weeks
      var weekStart = new Date(now);
      weekStart.setDate(weekStart.getDate() - weekStart.getDay() - (25 * 7));
      for (var w = 0; w < 26; w++) {
        var ws = new Date(weekStart);
        ws.setDate(ws.getDate() + w * 7);
        var we = new Date(ws);
        we.setDate(we.getDate() + 6);
        var wk = ws.getFullYear() + "-W" + String(w + 1).padStart(2, "0");
        periods.push({ key: wk, label: (w + 1), year: ws.getFullYear(), month: ws.getMonth(), start: ws, end: new Date(we.getFullYear(), we.getMonth(), we.getDate(), 23, 59, 59) });
      }
    }

    // Build interaction index per company per period
    var interactionMap = {}; // { company: { periodKey: [interactions] } }
    filtered.forEach(function(c) { interactionMap[c] = {}; });
    this.interactions.forEach(function(item) {
      if (!interactionMap[item.company]) return;
      var dateStr = item.date || "";
      if (!dateStr) return;
      var itemDate = new Date(dateStr + "T12:00:00");
      periods.forEach(function(p) {
        if (itemDate >= p.start && itemDate <= p.end) {
          if (!interactionMap[item.company][p.key]) interactionMap[item.company][p.key] = [];
          interactionMap[item.company][p.key].push(item);
        }
      });
    });
    ...
    var typeColors = {
      "email": "#74b9ff", "meeting": "#00b894", "call": "#fdcb6e",
      "follow-up": "#fdcb6e", "followup": "#fdcb6e",
      "technical": "#a29bfe", "note": "#a29bfe",
      "no response": "#e74c3c", "contract": "#2d3436"
    };
    ...
    // Render header row with period labels
    var html = '<div class="tl-table">';
    html += '<div class="tl-header-row">';
    html += '<div class="tl-col tl-col-company">Company</div>';
    html += '<div class="tl-col tl-col-country">Country</div>';
    html += '<div class="tl-col tl-col-stage">Stage</div>';
    html += '<div class="tl-col tl-col-last">Last Contact</div>';
    html += '<div class="tl-col tl-col-next">Next Follow-up</div>';
    html += '<div class="tl-col tl-col-timeline">';
    html += '<div class="tl-period-labels">';
    var lastYear = null;
    periods.forEach(function(p, i) {
      var yearLabel = "";
      if (self.timelineMode === "monthly" && p.year !== lastYear) {
        yearLabel = '<span class="tl-year-label">' + p.year + '</span>';
        lastYear = p.year;
      }
      html += '<span class="tl-period-cell' + (self.timelineMode === "monthly" && p.month === now.getMonth() && p.year === now.getFullYear() ? " tl-current" : "") + '">' + yearLabel + p.label + '</span>';
    });
    html += '</div></div></div>';
    ...
    filtered.forEach(function(c) {
      var bus = busMap[c];
      var stage = self.getCompanyStage(c);
      var latest = self.getLatestInteraction(c);
      var lastContact = latest ? latest.date : "—";

      // Find next follow-up from interactions
      var nextFu = "—";
      var fuItems = self.interactions.filter(function(i) { return i.company === c && i.followUp && !i.followUpDone; });
      fuItems.sort(function(a, b) { return (a.followUp || "").localeCompare(b.followUp || ""); });
      if (fuItems.length) nextFu = fuItems[0].followUp;

      var goClass = self.goStatus[c] === "nogo" ? " tl-row-nogo" : "";

      html += '<div class="tl-row' + goClass + '" data-company="' + self.escapeHtml(c) + '">';
      html += '<div class="tl-col tl-col-company"><strong>' + self.escapeHtml(c) + '</strong></div>';
      html += '<div class="tl-col tl-col-country">' + self.escapeHtml(bus ? bus.country || "" : "") + '</div>';
      html += '<div class="tl-col tl-col-stage"><span class="tl-stage-badge" style="background:' + stage.color + ';color:#000">' + self.escapeHtml(stage.stage) + '</span></div>';
      html += '<div class="tl-col tl-col-last">' + lastContact + '</div>';
      html += '<div class="tl-col tl-col-next">' + nextFu + '</div>';
      html += '<div class="tl-col tl-col-timeline"><div class="tl-grid">';

      periods.forEach(function(p) {
        var items = (interactionMap[c] && interactionMap[c][p.key]) || [];
        var count = items.length;
        if (count === 0) {
          html += '<span class="tl-cell tl-empty" title="No activity"></span>';
        } else {
          var color = getTypeColor(items[0].type);
          var sym = getCountSymbol(count);
          var tooltip = items.map(function(it) {
            return (it.date || "") + " | " + (it.type || "") + " | " + (it.subject || "") + (it.notes ? "\n" + it.notes.substring(0, 80) : "");
          }).join("\n");
          html += '<span class="tl-cell tl-filled" style="background:' + color + '" title="' + self.escapeHtml(tooltip) + '">' + sym + '</span>';
        }
      });
    ...
    var stageEl = document.getElementById("tlFilterStage");
    ...
    if (stageEl) stageEl.onchange = function() { self.timelineFilters.stage = stageEl.value; self.renderTimeline(); };
    if (goEl) goEl.onchange = function() { self.timelineFilters.go = goEl.value; self.renderTimeline(); };

    // Bind mode toggle
    document.querySelectorAll(".tl-mode-btn").forEach(function(btn) {
      btn.addEventListener("click", function() {
        document.querySelectorAll(".tl-mode-btn").forEach(function(b) { b.classList.remove("is-active"); });
        btn.classList.add("is-active");
        self.timelineMode = btn.dataset.mode;
        self.renderTimeline();
      });
    });

    // Bind row click to open company detail
    el.querySelectorAll(".tl-row").forEach(function(row) {
      row.addEventListener("click", function() {
        var tab = [...self.el.tabs].find(function(t) { return t.dataset.tab === "dashboard"; });
        if (tab) tab.click();
        self.showCompanyDetail(row.dataset.company);
      });
    });
  },
